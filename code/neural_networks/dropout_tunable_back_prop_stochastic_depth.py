...
def tune_alpha(depths,breathes,alphas=[0.1]):
    np.random.seed(1)
    X = np.array([[0,0,1],
                  [0,1,1],
                  [1,0,1],
...
def tune(depth,breathe,alpha,dropout_rate): 
    np.random.seed(1)
    X = np.array([[0,0,1],
                  [0,1,1],
                  [1,0,1],
    ...
    tmp = {"depth":depth,"breathe":breathe,"alpha":alpha}
    #print "With alpha=",alpha
    tmp["errors"] = train_network(X,y,depth,breathe,alpha=alpha,num_iterations=70000,dropout=True,dropout_percent=dropout_rate)
    tmp["min_error"] = min(tmp["errors"])
    tmp["ave_error"] = sum(tmp["errors"])/float(len(tmp["errors"]))

def analyze_results(tmp):
    print "The network had the following attributes"
    print "Depth",tmp["depth"]
    print "Breathe",tmp["breathe"]
    print "Alpha",tmp["alpha"]
...
def stochastic_tuning(depth_range,breathe_range,num_iter,alphas=[0.001,0.1,1,5,10,15,20,25,30,35,40],dropout_rates=[0.01,0.1,0.15,0.25,0.35,0.45,0.5,0.65,0.75,0.85,0.95]):
    results = []
    for iter in xrange(num_iter):
        depth = random.randint(0,depth_range)
        breathe = random.randint(0,breathe_range)
        for i in xrange(len(alphas)):
            for j in xrange(len(dropout_rate)):
            results.append(tune(depth,breathe,alphas[i],dropout_rate[j]))
    return results
